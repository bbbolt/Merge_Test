...
class AbsHandler(tornado.web.RequestHandler, ABC):
    ...
    def prepare(self):
        ...
        self.Cookie = self.request.headers.get("Cookie", self.request.headers.get("cookie", ""))
    ...
    def write(self, chunk, request_id="", x_rid="", stream=False):
        ...
        if not stream:
            ...
            try:
                super().write(chunk)
                super().flush()
            except (StreamClosedError, ValueError, OSError, AttributeError) as e:
                log.app_logger.warning(str(self.request_id) + " 写入异常: " + str(e))
                pass


class ThreadCorsHandler(AbsHandler, ABC):
    reg_allowed_origin = ".*.wps.cn[/]?$"
    executor = ThreadPoolExecutor(max_workers=int(MCP_MAX_WORKERS))
