...
class XBatchCompare(BatchCompare):
    ...
    def list_subdirectories(self, root_dir):
        ...
        for subpath in subdirectories:
            ...
            # 过滤掉包含 "##" 的路径
            val_subdirectories = [d for d in excel_file_paths if '执行样张' in d]
            run_subdirectories = [d for d in excel_file_paths if '新推理样张' in d]
    ...
    def run(self, all_pair_path, log_path, worker_number=1, code_dict=None):
        ...
        while result_queue:
            ...
            try:
                cur_res["index"] = os.path.basename(index[0]).split("_")[0]
            except:
...
class CheckCode():
    def __init__(self, args):
        ...
        # 获取当前日期和时间并添加log时间
        time_now = datetime.now().strftime("%Y-%m-%d_%H_%M")
    ...
    def run_ai(self, code, case_path, label_case_path, case_save_path):
        ...
        try:
            ws1 = self.load_workbook_with_timeout(label_case_path, 30).active
            sheet_name = ws1.title
            ws2 = self.load_workbook_with_timeout(case_save_path, 30)[sheet_name]
        except:
            return (False, "执行样张读取异常（可能超长或存在异常值）") if (response["value"] == 'null') else (
                False, response["value"])
        # 获取最大行和最大列
        max_row = max(ws1.max_row, ws2.max_row)
        max_column = get_column_letter(ws1.max_column) if ws1.max_column > ws2.max_column else get_column_letter(
            ws2.max_column)
        # 获取最小行和最小列
        min_row = min(ws1.min_row, ws2.min_row)
        min_column = get_column_letter(ws1.min_column) if ws1.min_column > ws2.min_column else get_column_letter(
            ws2.min_column)
        ...
        shutil.copy(self.replace_excel(case_path), tgt_dir)
        shutil.copy(self.replace_excel(label_case_path), tgt_dir)
    ...
    def apply_code(self, original_xlsx_file_path,label_xlsx_file_path, code, processed_xlsx_save_path):
        run_flag = self.run_ai(code, original_xlsx_file_path, label_xlsx_file_path, processed_xlsx_save_path.replace("原始样张", "新推理样张"))
        return run_flag
    ...
    def list_subdirectories(self, root_dir):
        ...
        subdirectories = glob.glob(pattern)
        # 过滤出文件夹
        subdirectories = [path for path in subdirectories if os.path.isdir(path)]
        # 过滤掉根目录本身
        subdirectories = [d for d in subdirectories if d != root_dir]
        for subpath in subdirectories:
            # 分别获取 .xlsx 和 .xls 文件
            ...
            val_path = [d for d in excel_file_paths if '执行样张' in d and not os.path.basename(d).startswith("~$")]
    ...
    def match_and_package(self, cases_path, json_data):
        ...
        with open(json_data, "r", encoding='utf-8') as file:
            for line in file:
                ...
                md5 = json_dict[self.data_mapping_dict["md5"]]
                input = json_dict[self.data_mapping_dict['input']]
                target = json_dict[self.data_mapping_dict['target']]
                question = re.findall(r"需求“(.*?)”", input, re.DOTALL)
                result[os.path.join(cases_path, md5)] = (question, input, target)
        return result
    ...
    def collection_result(self):
        ...
        self.order_list = ['用例编号', '样张路径', '原始样张', '校验样张', '执行样张', '问题', 'Prompt', 'AI返回JS', "表单属性差异", "单元格属性差异",'代码执行成功','运行信息','校验是否通过']
    ...
    def main(self):
        ...
        # 使用tqdm显示进度条
        with tqdm(total=total_num, desc="生成新推理样张结果: ") as pbar:
            for key in result:
                index = os.path.basename(key)
                orig_case_name = os.path.basename(all_pair_path[key][0])
                orig_case_path, right_case_path, result_case_path = orig_case_name, orig_case_name.replace("原始样张", "执行样张"), orig_case_name.replace("原始样张", "新推理样张")
                ...
                try:
                    ...
                    code_miss += 1
                    collection_dict_gen_sheet[key] = ("问题无对应代码", [orig_case_path, right_case_path, ""], ("", "", ""), "NULL")
                    self.logger.info(f"问题无对应代码: {key}")
                    continue
                if len(all_pair_path[key])==1:
                    case_right_miss+=1
                    collection_dict_gen_sheet[key] = ("校验样张丢失", [orig_case_path, "", ""], result[key], "NULL")
        ...
        self.logger.info(f"总验证: {total_num} | 正确执行: {success} | 校验样张丢失: {case_right_miss} | 问题无对应代码: {code_miss}")
...
def parse_arguments(kas_checkpoint_path, src_path, base_url, client_path, data_mapping_dict):

    name = os.path.basename(src_path)
    infer_parser = argparse.ArgumentParser(description='Infer Config')
    infer_parser.add_argument('--model_path', type=str, help='model_path', default=kas_checkpoint_path)
    infer_parser.add_argument('--base_url', type=str, help='server_URL', default=base_url)
    infer_parser.add_argument('--local_data_path', type=str, help='json_data_path', default=rf"{src_path}\run_code_result\{name}_eval.json")
    infer_parser.add_argument('--infer_result_save_path', type=str, help='infer_result_save_path', default=rf"{src_path}\run_code_result\{name}_eval_infer_result.json")
    infer_parser.add_argument('--number_worker', type=int, help='number_worker', default=16)
    infer_parser.add_argument('--data_mapping_dict', type=str, help='data_mapping_dict', default=data_mapping_dict)
    infer_args = infer_parser.parse_args([])

    val_parser = argparse.ArgumentParser(description="Validation")
    # Basic configuration
    val_parser.add_argument('--client_path', type=str, default=client_path, help='Client service path (execution path)')
    # Dataset configuration
    val_parser.add_argument('--cases_path', type=str, default=rf"{src_path}\run_code_result", help='Test cases path')
    # Validation configuration
    val_parser.add_argument('--target_path', type=str, default=rf"{src_path}\eval_results", help='Target path for validation results')
    val_parser.add_argument('--json_file_path', type=str, default=rf"{src_path}\run_code_result\{name}_eval_infer_result.json", help='Path to the JSON file')
    val_parser.add_argument('--worker_number', type=int, default=16, help='Number of workers. Default: 16')
    val_parser.add_argument('--data_mapping_dict', type=str, help='data_mapping_dict', default=data_mapping_dict)

    # Log configuration
    val_parser.add_argument('--log_path', type=str, default=rf"{src_path}\logs", help='Path to the log files')

    val_args = val_parser.parse_args([])


    return infer_args, val_args

if __name__ == "__main__":

    parser = argparse.ArgumentParser(description='Infer Config')
    # Model
    parser.add_argument('--kas_checkpoint_path', type=str, help='kas_checkpoint_path', default="/home/kas/kas_workspace/open_source_llm/Qwen2.5-3B-Instruct")
    # Src Path
    parser.add_argument('--src_path', type=str, help='src_path', default=r"C:\Users\Bolt\PycharmProjects\TelunXuptTry\et-code-eval\gen_sheets\jsapi0221_w_think_clean_RANGE_FORMAT")
    parser.add_argument('--client_path', type=str, help='client_path', default=r'C:\Users\Bolt\Downloads\office6\office6')
    parser.add_argument('--base_url', type=str, help='server_URL', default='http://127.0.0.1:8002')
    parser.add_argument('--data_mapping_dict', type=str, help='data_mapping_dict', default='{"input": "prompt", "target": "answer", "md5": "md5"}')
    args = parser.parse_args()

    infer_args, val_args = parse_arguments(args.kas_checkpoint_path, args.src_path, args.base_url, args.client_path, args.data_mapping_dict)
    # infer_base_vllm_func(infer_args)
