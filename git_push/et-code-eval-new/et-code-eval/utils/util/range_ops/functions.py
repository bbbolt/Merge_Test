...
FUNCTIONS = {
    ...
    "rop_get_context": '''function rop_get_context(task)
{
    let context = {}
...
function FindDelete(targetRange, task, isRow){
    ...
    if (currentRange == undefined || currentRange == null) {
        currentRange = ActiveSheet.UsedRange
    }
...
function FindContentDo(currentRange, values, lookAt, doFunc)
{
    ...
    {
        let value = values[i]
        let intersect = Intersect(currentRange, ActiveSheet.UsedRange)
        if (intersect == null)
            doFunc(currentRange)
        ...
        let dealAddressSet = new Set();
        dealAddressSet.add(cellResult.Address())
        while (cellResult != undefined)
        {
            ...
            if (cellResult == undefined)
                break
            if (dealAddressSet.has(cellResult.Address())){
                break
            } else
                dealAddressSet.add(cellResult.Address())
        }
...
function FindReplace(task)
{
    ...
    function doReplaceCell(range, obj = task)
    {
        ...
        if (obj.src_value == "" || obj.src_value == null)
...
''', 
    "doBeautify": '''function doBeautify(range, type, bSetTableStyle, tableStyleName)
{
    targetRange = range
    if (range == null)
        targetRange = ActiveCell.CurrentRegion
    
    // 设置字体
    setFontStyle(targetRange)
    
    targetRange.Columns.AutoFit()
    targetRange.Rows.AutoFit()

    for(let col = 1; col <= targetRange.Columns.Count; col++)
    {
        let colRange = targetRange.Columns.Item(col)
        let colWidth = colRange.ColumnWidth

        //宽度超过50，折行
        if (colWidth >= 50)
        {
            colRange.WrapText = true
            colRange.ColumnWidth = 50
        }
        else
        {
            colRange.ColumnWidth = colWidth + getColSpacing(colWidth, type)
        }
    }
    ...
    try
    {
        ActiveSheet.ListObjects.Add(xlSrcRange, null, null, xlGuess, null, "TableStyleLight1")
        targetRange.ListObject.ShowHeaders = false
        if (!tableStyleName)
            return
        targetRange.ListObject.TableStyle = tableStyleName
    }
    catch
    {}
...
function setFontStyle(range) {
    ...
    range.Font.Size = 11;
    range.Font.Color = RGB(51, 51, 51);
    
    // 为第一行（表头）设置特殊样式
    let headerRow = range.Rows(1)
    headerRow.Font.Bold = true
    headerRow.Font.Size = 12
    headerRow.Font.Color = RGB(0, 0, 0)
}''',
    "getColSpacing": '''function getColSpacing(colWidth, type)
{
    if (type == "adapt")
    {
        if (colWidth < 15)
...
# 工具函数和对应依赖函数列表
RANGE_OPS_DATA_MAPS = {
    "CLOPConcat": ["CLOPConcat", "ConcatCellsValue", "GetUnionTableRange", "rop_get_context"],
    "CLOPFindReplace": ["GetSpecialCellEnum", "GetUnionTableRange", "Find", "FindContent", "CLOPFindReplace", "FindReplace", "FindContentDo", "IsCellAddress", "rop_get_context"],
    "CLOPColumnsInsert": ["IllegalBooleanParameter", "GetColumnRowRange", "InsertNullRowOrCol", "GetUnionTableRange", "CLOPColumnsInsert", "IllegalStringParameter", "rop_get_context"],
    "CLOPDeleteRow": ["IllegalBooleanParameter", "FindDelete", "CheckEmptyRowOrColumn", "Find", "FindContent", "GetSpecialCellEnum", "GetColumnRowRange", "IsCellAddress", "GetUnionTableRange", "IllegalStringParameter", "CLOPDeleteRow", "rop_get_context"],
    "CLOPRowsInsert":["GetSpecialCellEnum", "IllegalStringParameter", "Find", "InsertNullRowOrCol", "GetColumnRowRange", "CLOPRowsInsert", "FindRowsInsert", "IsCellAddress", "FindContent", "GetUnionTableRange", "findRangeByContent", "IllegalBooleanParameter", "rop_get_context"],
    "CLOPDeleteCol": ["IllegalStringParameter", "CLOPDeleteCol", "GetColumnRowRange", "CheckEmptyRowOrColumn", "GetUnionTableRange", "FindDelete", "IllegalBooleanParameter", "rop_get_context"],
    "CLOPDeleteValue": ["CLOPDeleteValue", "DeleteCellsValue", "IllegalBooleanParameter", "GetUnionTableRange", "IllegalStringParameter", "rop_get_context"],
    "doBeautify": ["doBeautify", "getColSpacing"],
    "rop_get_context": ["rop_get_context"]
