...
import re
from json_to_js.hyperlink.functions import FUNCTIONS, FUNCTIONS_MAP, MARCO_FUNC, TASK_CODE, CONTEXT_CODE
...
def get_js_code(input_json_lst):
    ...
    for idx, input_json in enumerate(input_json_lst):
        ...
        if "SubAddress" in input_json:
            if "!" in input_json["SubAddress"]:
                task_para_code = task_para_code.replace("''", "'\\'").replace("'!", "\\'!")
        context_code = "    " + "let [task_{}_{}, targetWorksheet, targetWorkbook] = get_context(task_{})".format(idx, idx, idx) + '\n'
...
if __name__ == '__main__':
    ...
    file_path = "/home/kas/kas_workspace/liuyujia/copilot/data/aug_data/hyperlink/newtable_json"
